function [minloc_row, minloc_col] = MGLM_Gross_spd_cv(X, Y, Xval, Yval, lambdaSet, rhoSet, opts)
% MGLM_GROSS_SPD_CV performs cross validation to choose the optimal
...
for j=1:Numlambda
    for k=1:Numrho
        ...
        [phat, Vhat, ~, ~, ~, ~] = MGLM_Gross_spd(X, Y, opts);
