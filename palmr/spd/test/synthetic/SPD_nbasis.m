% experiments on SPD manifolds investingating the effect of nbasis, the
...
for i = 1:nrepeat
    ...
    for j = 1:length(nbasisSet)
        para.nbasis = nbasisSet(j);
        ...
        Xval = randn(para.nbasis, nval);
        Yval = predSPD(p, V, Xval);
        
        % generate testing data
        Xte = randn(para.nbasis, ntest);
        Yte = predSPD(p, V, Xte);
        ...
        % 'Numerical Error'. In such case, we can use palmr with specific
        % parameters as an alternative choise, since MGLM is a specical
        % case of PALMR when lambda = 0 and rho = inf.
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         try
...
%         end
    ...
    save ../../results/SPD_nbasis_result.mat OutputNoG OutputWithG;
end
